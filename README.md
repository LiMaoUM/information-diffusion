...
| Path | Contents |
|---|---|
| `Information_Diffusion_ICWSM/` | Paper: anonymous submission LaTeX, restructured draft, camera-ready template, figures, copyright forms |
| `figures/` | All generated manuscript figures (scaling, CCDFs, motifs, regression diagnostics) |
| `src/bsky_api.py`, `bsky_follows.py` | Bluesky data collection (threads, reposts, profiles, follow graph) |
...
| `src/model_R.R` | Robust regression (lmrob/rlm/GLS) of log breadth on log size by platform |
...
1. Collect posts, reposts, threads, follows (`src/bsky_api.py`, Truth Social equivalents).
...
4. Regression modeling in R (`model_R.R`).
