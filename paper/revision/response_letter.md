...
6. **Errors in the submitted version are corrected**: the inconsistent description of the regression estimator, a degenerate scale estimate that affected Table 1 and two appendix tables, and a main text claim of a million motif randomizations where the code and the appendix both say 100.
7. **Claims are calibrated to what the models show.** Ideological structure now "accounts for most of" the platform divergence rather than making it "effectively disappear", the repost results are "robust to the reconstruction rules examined here" rather than "invariant to the reconstruction rule", and H3 is adjudicated once, as not supported, since adding topic leaves the platform by size interaction unchanged.
...
Because this choice affects depth and breadth, we rebuilt every reply cascade from the raw thread data and recomputed all metrics under both representations. The post level reconstruction reproduces the analysis file (correlations of 0.999 for size, 0.999 for breadth, 0.988 for depth), confirming the reported analyses are post level. Both representations are compared within a common subsample, the cascades that actually contain a reply (47,106 at post level, 42,947 after collapsing), and fitted with the same Huber specification as Table 1; the appendix now says so, since the coefficients there are not comparable in level to a main table estimated on all 123,189 cascades. Under user collapsing, the depth divergence persists and is again absorbed by ideology, but the breadth divergence is much smaller. We interpret and report this directly: part of the raw breadth gap reflects the same Truth Social users posting repeatedly within a thread rather than a wider set of participants. The depth contrast, which the discussion rests on, holds under both representations.
...
On motif dependence, which Reviewer 1 raised: overlapping motifs do not bias the z-scores, because the randomized graphs are counted with the identical enumeration, so overlap inflates observed and null counts alike. We now describe the null model precisely, including that rewiring happens within each cascade and preserves cascade sizes, out degrees, and each participant's ideology, and that we draw 100 randomized graphs per platform with 5m swap attempts in a cascade of m edges. The submitted main text reported a million randomizations, which did not match either the appendix or the code; the correct number is 100 and both now state it. We also state the real consequence of the dependence, which is that neighboring motif z-scores are correlated and should be read as a pattern across families rather than as independent evidence.
...
Re-estimating the table surfaced two further errors in the submitted version, which we have corrected. The footnote to Model 2 stated that the interaction was with an outlier indicator; the model is in fact a three category platform variable (Bluesky, Truth Social non influencer, Truth Social influencer) and the reported entry is the Truth Social non influencer contrast against Bluesky. Model 4 for breadth was reported as 0.4286 while its own fit statistics correspond to 0.4086, and the Model 4 depth row did not reproduce from the stated specification. All Model 4 entries are now re estimated on the same sample and estimator as the other models, giving 0.426 for breadth and -0.330 for depth. Neither correction affects the conclusion that topic does not account for the platform gap.

The same scale problem affected two appendix tables that we had not initially rechecked, and we have re-estimated both. The ideology threshold robustness table (Table 2) and the composition encoding comparison (Table 7) were fitted with the default scale and are now fitted with Huber's proposal 2, on the same specification as Table 1. The threshold table's conclusion is unchanged and is now visible in the numbers rather than obscured by them: the residual platform by size interaction is 0.155 and -0.107 at a 0.5 threshold, 0.066 and -0.045 at 0.6, and -0.008 and 0.013 at 0.7, shrinking as a stricter threshold moves more users into the residual center category. The encoding comparison now shows that neither encoding dominates on fit, and that recoding composition by each platform's dominant ideology gives a smaller residual interaction in the composition only models and a larger one once alignment is included. We retain left/right because its coefficients carry a fixed ideological meaning across platforms. Table 7 also now reports the size main effect and the sample size, and its estimator note has moved into the table notes.

**Changes**: corrected appendix paragraph; new section "Sensitivity of the Modeling Specification" with Table 6; Tables 2 and 7 re-estimated under Huber's proposal 2.
