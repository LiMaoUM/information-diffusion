...
*On the size of the validation sample.* Rather than adding items only to the rare classes, which would make the validation set unrepresentative of the data it validates, we report bootstrap intervals so that the precision of each estimate is visible. Those intervals show where the evidence is strong and where it is not: the pooled rates and both majority classes are tightly estimated, while the two minority cells rest on 8 and 17 items and are correspondingly wide, from 0.33 to 0.91 for right on Bluesky. We state this in the paper rather than presenting all nine cells as equally well established. We are also expanding the annotation modestly, without stratifying toward the rare classes, and will report whether the rates move.
...
All three are addressed in SPC-3: per class and per platform metrics are now reported, the center category is defined as a residual and tested by refitting without it, and a label noise simulation shows the conclusions survive errors at the measured rates. We also corrected agreement statistics that did not match our archived annotation file. We agree that 200 items is a small sample for per class per platform estimates. Rather than adding items only to the rare classes, which would leave the validation set unrepresentative of the data it validates, we report bootstrap intervals for every rate so a reader can see which cells are well estimated and which are not, and we are expanding the annotation modestly without stratifying toward the rare classes.
