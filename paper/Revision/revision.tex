...
\begin{figure}[tb]
    ...
    \caption{(A--B) Cascade breadth (A) and depth (B) against cascade size for reply cascades on BlueSky and TruthSocial, both log-transformed. (C--D) The same for repost cascades. Points are individual cascades; lines are fitted values. \rev{Each panel fits one pooled model, $\log Y \sim \log N \times \text{platform}$: the BlueSky line is the size slope and the TruthSocial line adds the interaction, so the slopes in (A--B) are those of the baseline model in Table~\ref{tab:model_comparison}. Panels (A--B) use the 123{,}189 reply cascades (Huber loss, Huber's proposal 2 scale); (C--D) the 244{,}112 repost cascades with at least one repost (OLS, HC3).}}
    \label{fig:bd}
\end{figure}
