...
\caption{\rev{Class-specific model performance against human consensus labels (186 of 200 validation items on which both annotators agree). $n$ is the consensus support per class.}}
...
\rev{Second, part of the gap is measurement error, and our validation lets us estimate how much. The classifier over-assigns each platform's minority position (Table~\ref{tab:validation}). Applying a standard prevalence correction, which inverts the measured confusion matrix to recover the label distribution implied by the observed one, lowers the estimated right-leaning share on Bluesky from 24.2\% to 6.9\%, close to the 4.8\% reported for the platform as a whole, and lowers the estimated left-leaning share on Truth Social from 11.5\% toward zero. These corrected values should be read as indicative rather than precise, since the correction inherits the imprecision of the minority cells in Table~\ref{tab:validation}. Their direction is nonetheless clear and consistent with the reviewer's expectation: our raw labels overstate each platform's ideological minority, the platforms are more homogeneous than the uncorrected shares suggest, and the label-noise analysis above shows the structural conclusions survive errors of this magnitude.}
