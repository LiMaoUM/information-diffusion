...
Fine-grained motif analysis further reveals asymmetric interaction patterns, including distinct roles for left-, right-, and center-leaning users across platforms. \rev{Overall, the results suggest that political information can be amplified similarly across platforms even when conversational engagement is organized differently across ideological groups.}
...
Similarly, certain topics can be more contentious, leading to different diffusion patterns \cite{vosoughi_spread_2018} and topic distribution can vary across platforms. Interestingly, we find neither of these explain the reply scaling differences between the two platforms. Ideological composition partially explains the differences and, most importantly, when accounting for both composition and the degree to which users engage in cross-ideological discussions, \rev{most of the platform-level difference is accounted for.} 
...
We model information diffusion as directed acyclic graphs (DAGs), where \rev{each node is a single post (the root post, a reply, or a repost)} and edges represent inferred diffusion pathways, following the time-inferred approach of \citet{vosoughi_spread_2018}. \rev{Each node is thus one action by a user, and a user who replies several times in one thread contributes one node per reply. We keep these appearances separate because a reply tree records how a conversation actually unfolded; merging them would add a modeling assumption to observed data. Ideology is aggregated at the user level: each node carries the label (left-leaning, center, or right-leaning) of the account that wrote it (see Ideology Estimation and Alignment), so a cascade's ideological composition is the share of its posts written by accounts in each category.} For \textbf{reply cascades}, trees were constructed directly from reply metadata: each reply is linked to its parent post, and cascades are rooted at the original post with subsequent replies linked recursively. For \textbf{repost cascades}, platform APIs report all reposts as nominally linking to the original post, which obscures the true diffusion path. To approximate true repost trees, \rev{we linked each repost $r_i$ to its earliest possible source, as in the ``first influence'' rule of \citet{bakshy_everyones_2011}: the original post if the user who made $r_i$ is observed to follow its author in the follower snapshot, and otherwise the earliest prior repost $r_j$ with $t_j < t_i$ made by a user they are observed to follow, giving the edge $r_j \rightarrow r_i$}. When no such user existed, the repost was attributed directly to the original post. \rev{Because these edges are inferred and reconstruction assumptions can distort cascade properties \citep{deverna_cascade_2024}, we refer to repost cascades as reconstructed throughout and, as in that earlier work, verify that the results hold under alternative attribution rules (Appendix, Robustness of Repost Cascade Reconstruction). The follower network is a snapshot without edge creation dates, collected together with the posts and reposts, one day apart, and at the same time for both platforms.}

\subsection{Cascade Metrics}
For each cascade we compute four structural metrics. \textbf{Size} ($N$) is the \rev{total number of posts in the cascade, including the root. Descriptive counts for repost cascades (Tables~\ref{tab:descriptives_main} and~\ref{tab:descriptives}) report reposts, excluding the root}. \textbf{Breadth} ($B$) is the maximum number of nodes at any single depth level: $B = \max_{d \geq 1} n(d)$, where $n(d)$ is the node count at depth $d$. This captures the widest ``layer'' of the cascade and differs from root out-degree, which counts only direct children of the root. \textbf{Depth} ($D$) is the maximum hop distance from the root to any leaf node: $D = \max_{v} \mathrm{dist}(\mathrm{root}, v)$. \textbf{Structural Virality}~\cite{goel_structural_2016} is the average pairwise shortest-path distance between all nodes in the cascade, distinguishing broadcast-like (low virality) from chain-like viral (high virality) spread. \rev{For modeling we take $\log_{10} N$, $\log_{10} B$ and $\log_{10}(D+1)$, the last offset because a cascade with no replies has $D=0$. Such a cascade has $N=B=1$ and $D=0$, so all three values are zero and it sits at the origin of the scaling plots; 61.3\% of reply cascades are of this kind, and all are retained in Table~\ref{tab:model_comparison}.}
\begin{figure*}
    \centering
...
We used the \texttt{Llama-3.1-8B-Instruct} model to classify each reply into one of five ideological stances (\textit{left}, \textit{lean-left}, \textit{center}, \textit{lean-right}, \textit{right}), providing the original post and preceding replies as context. For robustness, we grouped stances into three categories: \textit{left-leaning} (left, lean-left), \textit{center}, and \textit{right-leaning} (lean-right, right). Annotations were validated against two human annotators on a random sample of 200 replies\rev{. Agreement between the model and each annotator ($\kappa = 0.61$ and $0.74$) is largely comparable to agreement between the two annotators ($\kappa = 0.77$), and the robustness checks in Appendix~\ref{app:ideology_distribution} show that our results hold under alternative ideology thresholds and under label errors at the measured rates}. Because large language models may encode biases, we treat these labels as probabilistic indicators rather than ground truth.

\paragraph*{User-Level Ideology Inference.}
User ideology was inferred by aggregating stance labels across all posts authored by each user. A user was classified as \textit{left-leaning} if more than 60\% of their posts were labeled left or lean-left, \textit{right-leaning} if more than 60\% were labeled right or lean-right, and \textit{center} otherwise. \rev{Center-labeled users are therefore those whose posts alternate between left and right, together with those who consistently post without an overt ideological stance.} This threshold balances ideological signal with label consistency; aggregate patterns are robust across thresholds from 0.5 to 0.7 (see Appendix~\ref{app:ideology_distribution}). 
...
For each reply edge, we code it as \textit{aligned} if both users share the same ideology and \textit{misaligned} otherwise. The alignment ratio for a cascade is the proportion of aligned reply edges. \rev{A cascade with no reply edges has no such proportion, and we set its alignment ratio to zero.} High ratios indicate ideologically homogeneous discussions; lower ratios reflect cross-cutting exchanges. \rev{In this scenario, consider a thread whose root post is by a right-leaning user $A$. Two right-leaning users, $B$ and $C$, reply to $A$; a left-leaning user $D$ also replies to $A$; and another left-leaning user $E$ replies to $D$. The replies from $B$, $C$, and $E$ are aligned and the reply from $D$ is misaligned, so the alignment ratio is $3/4 = 0.75$. The measure describes the conversation: the same user can contribute aligned edges in one thread and misaligned edges in another.} 
...
\begin{equation}
\Delta = \rev{1 - \frac{|\hat{\beta}_{3,\text{aug}}|}{|\hat{\beta}_{3,\text{base}}|}} ,
\end{equation}
where $\hat{\beta}_{3,\text{base}}$ is the interaction coefficient from Eq.~\ref{eq:eq1} and $\hat{\beta}_{3,\text{aug}}$ is the corresponding estimate from augmented specifications (Models 2--4). A larger $\Delta$ indicates that added covariates account for a greater portion of cross-platform difference. \rev{We report the proportional reduction in $|\beta_3|$; the tables report signed coefficients.}
...
Building on user-level ideology labels, we include alignment ratio (spline-transformed, df\,=\,3, to capture nonlinear effects at extremes of homogeneity) and \rev{the proportions of posts in each reply tree authored by left- and right-leaning users, with a spline for the left-leaning post share and a linear term for the right-leaning post share; Appendix~\ref{app:spline} reports sensitivity to alternative functional forms}. \rev{Both measures enter as main effects and interacted with cascade size.} We decompose this into three specifications: composition only (Model 3a), alignment only (Model 3b), and combined composition and alignment (Model 3c). 

\paragraph*{Evaluating Topic Variability.}
To address H3, we include cascade-level topic annotations as categorical moderators \rev{interacted with $\log(\text{size}_i)$}, testing whether content differences account for cross-platform variation in how depth and breadth scale with size.
...
\begin{figure}[tb]
    ...
    \caption{(A--B) Cascade breadth (A) and depth (B) against cascade size for reply cascades on BlueSky and TruthSocial, both log-transformed. (C--D) The same for repost cascades. Points are individual cascades; lines are fitted values. \rev{Each panel fits one pooled model, $\log Y \sim \log N \times \text{platform}$.}}
    \label{fig:bd}
\end{figure}
...
\rev{Table~\ref{tab:descriptives_main} gives the counts behind these distributions. Most Bluesky reply cascades (71.3\%) are a single post, against 43.3\% on Truth Social, and 9.0\% of Truth Social reply cascades exceed 50 posts, against 0.3\% on Bluesky; repost cascades are more alike in size.}
...
\toprule
Type & Platform & Cascades & Posts & Size 1 & $>$50 \\
\midrule
Reply & Bluesky & 79{,}348 & 199{,}413 & 71.3\% & 0.3\% \\
Reply & Truth Social & 43{,}841 & 1{,}369{,}497 & 43.3\% & 9.0\% \\
Repost & Bluesky & 25{,}550 & 184{,}807 & 46.3\% & 2.3\% \\
Repost & Truth Social & 218{,}579 & 3{,}847{,}903 & 51.4\% & 5.0\% \\
\bottomrule
\end{tabular}
}
\caption{\rev{Cascade counts and sizes. ``Size 1'' is the share of cascades with a single node: one post for replies, one repost for reposts. Reply counts include roots; repost counts exclude them. Full distribution in Appendix Table~\ref{tab:descriptives}.}}
...
End-shift transitions (e.g., C--C--R, L--L--C) do not follow a uniform pattern. On Bluesky, end-shifts are generally at or below the rewired baseline, with L$\leftrightarrow$C links most underrepresented. On Truth Social, three end-shift motifs are overrepresented (R--C--C, R--R--L, L--C--C);
...
\rev{each brings a center- or left-labeled user, the platform's minorities, into a chain, for example a left-leaning reply at the end of a right--right exchange.}
...
Truth Social also shows overrepresentation of several mixed configurations (L--R--R, L--C--C, C--R--R, R--L--L). \rev{Most place a numerically rare ideology at the root and a more common pair among the repliers, which on Truth Social means left-labeled roots with right-labeled repliers.} 
...
\section{Discussion}
This study compares how candidate-centered conversations about the 2024 U.S. presidential election unfold on two ideologically distinct but technically comparable platforms, Truth Social and Bluesky. Whereas most prior work emphasizes how far or how fast information spreads, we focus on structural form: whether interactions broaden outward through shallow branching or deepen through sequential layering. By analyzing both reposts and replies, we show that reposts scale similarly across platforms, but reply cascades diverge sharply: Truth Social supports broader, shallower exchanges, while Bluesky produces narrower, deeper threads. \rev{These results show that platforms with similar reply and repost affordances can exhibit different conversational dynamics, depending on the social and ideological composition of the communities that use them.}
...
Motif results highlight the combined role of interactional structure and user composition. On Truth Social, \rev{left-leaning users are a small minority of the users we observe}, yet appear disproportionately in Left$\leftrightarrow$Right bounce-back motifs: these are strongly overrepresented relative to the null model. \rev{This is why the model needs composition as well as alignment: how often exchanges cross ideological lines also depends on how many users of each ideology take part, which is what composition measures.}

\rev{Center-labeled users appear disproportionately in different local configurations across the two platforms. On Bluesky they occur more often in bounce-back motifs, as the targets of cross-ideological replies; on Truth Social they appear more often in several end-shift configurations.}

\rev{A further implication is that diffusion differs not only in reach, speed, or volume, but also in the structure of participation.} A cascade can become large through broad, shallow attention or through deeper sequential exchange, and these forms imply different kinds of political engagement. This distinction is especially important in a fragmented platform ecosystem, where newer platforms may share familiar Twitter-like affordances while supporting different conversational dynamics. By comparing reposts and replies separately, our study shows that amplification can scale similarly across platforms even when conversation diverges. Political diffusion should therefore be understood not only as whether information spreads, but as how participation is organized once it does.

\rev{The two cascade shapes represent different forms of conversational organization. Wide, shallow reply cascades consist of many users responding to the same post with little subsequent exchange among them; narrower, deeper cascades involve replies that themselves receive replies across multiple turns. Neither is inherently more desirable, but they distribute attention differently, and the difference arises here between platforms with similar reply mechanics.}

\rev{Methodologically, comparisons based on aggregate activity or cascade size alone would record Truth Social as simply more active and would miss this divergence entirely, because it appears only after conditioning on size. Structural measures of the kind used here are available wherever thread structure is recorded, and separating the diffusion patterns that recur across platforms from those that track the composition of a particular community is what makes findings portable to the next platform.} 


\rev{In our data, repost cascades scale similarly across platforms, whereas differences in reply structure are closely associated with ideological composition and interaction alignment.}
...
%% -------------------------------------------------------
\appendix
% let the appendix place more floats per page, so tables stay near their text
...
We re-estimated Model~3 using alternative thresholds of 0.5 and 0.7 for classifying user ideology. \rev{Figure~\ref{fig:threshold} shows how the share of users in each category changes with the threshold: center-labeled users remain at every threshold, including 0.5, and grow as the threshold rises. Table~\ref{tab:robust_threshold} reports the fit and the platform-by-size interaction at each threshold. Against the baseline model without ideology measures, whose platform interaction on this sample is $0.417$ for breadth and $-0.321$ for depth, composition and alignment account for 63\% and 67\% of the platform difference at a threshold of 0.5, 84\% and 86\% at 0.6, and 98\% and 96\% at 0.7. The share rises with the threshold because a stricter threshold sorts more users into the center category, which composition then separates from the partisan groups. Pseudo-$R^2$ values are uniformly high and error metrics change little.} We also applied an unsupervised natural breakpoint detection algorithm to the ideology distribution, which identified 0.67 as a statistically distinct split point; we adopt 0.6 as our default to avoid excessive sparsity.

\begin{figure*}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/threshold.png}
    \caption{User ideology proportions and consistency across thresholds on \textit{Bluesky} and \textit{Truth Social}. Left-leaning users dominate at lower thresholds on Bluesky; right-leaning users represent the majority on Truth Social at all thresholds. Center-labeled users increase at higher thresholds on both platforms. \rev{A user is labeled left- or right-leaning when that share of their posts reaches the threshold, the same rule used in the models.}}
    \label{fig:threshold}
\end{figure*}
...
\rev{All agreement figures use the same 200 validation replies. The two annotators agree at Cohen's $\kappa = 0.77$ (bootstrap 95\% interval $[0.70, 0.85]$), and the model agrees with them at $\kappa = 0.61$ (Annotator 1) and $0.74$ (Annotator 2). For precision and recall we treat the human label as ground truth, using the 171 items on which both annotators agree; Table~\ref{tab:validation} reports precision with bootstrap intervals, and recall, by class and platform, and overall accuracy against this consensus label is 0.86.}

\rev{The errors are not uniform across classes, and the pattern matters for interpreting our composition estimates: the model over-assigns each platform's ideological minority. Precision is 0.64 for right-leaning labels on Bluesky and 0.64 for left-leaning labels on Truth Social, against 0.98 and 0.93 for the largest class on each platform. Because the minority-class estimates rest on relatively few consensus cases, we distinguish between the direction of the observed bias and its precise magnitude: the two minority cells rest on 8 and 17 consensus items and their precision intervals are correspondingly wide (for right on Bluesky, $[0.33, 0.91]$). Bootstrapping the difference between minority-class and majority-class precision within each platform gives $-0.34$ on Bluesky (one-sided $p = 0.017$) and $-0.29$ on Truth Social ($p = 0.004$), so the direction is supported on both platforms. Treating each annotator's labels alone as ground truth, instead of the consensus, gives lower precision throughout, but the minority labels remain the least precise on both platforms. Uneven performance across the ideological spectrum is a documented property of language-model annotators \citep{vallejovera_llms_2025}.}

\rev{The misclassified items point to two sources of error. First, the model sometimes treats a post that describes the right as a right-leaning stance, and a post that describes the left as a left-leaning one. What a post discusses and where its author stands are related in complex ways: a description of the right can come from either side, and so can criticism of it, since a reply complaining that Republican leaders are too soft on immigration criticizes the right from the right. Second, some replies carry little substantive content. A reply such as ``What do we do?'' can be interpreted as supportive or as critical of the post it answers, depending on who writes it, which leaves room for error even with the preceding posts as context.}
...
\rev{Because the stance labels contain measurement error, we tested whether the central result survives label noise at the measured rates. Misclassification by automated classifiers biases downstream regression even when overall accuracy is high \citep{teblunthuis_misclassification_2024}, and the same holds for language-model labels used as surrogates \citep{egami_imperfect_2023}. Those authors correct the bias analytically with a gold-standard subsample; we instead propagate the measured error through the full pipeline, because our labels enter through composition, alignment, splines, and interactions rather than as a single regressor. For each of 100 draws per scenario, every user's label was replaced by a draw from the measured confusion distribution, cascade-level composition and alignment were recomputed, and Model 3c was refit. We use the confusion matrix because it measures the model's errors against human labels; the model's own confidence in each post has not been validated against human judgment. The matrix gives the distribution of human labels for each model label, $P(\text{human} \mid \text{model})$, estimated on the validation replies and applied to user-level labels. The simulation therefore treats reply-level error rates as a sensitivity model for the aggregated user labels, rather than as directly measured user-level error rates.}

\rev{We ran this under four error processes: the error rates measured on each platform; one set of error rates pooled across both platforms, which applies to each platform's dominant class an error rate measured mostly on the other platform; error rates re-estimated from a resampled validation sample in each draw, which carries the uncertainty of the small validation sample through; and the error rates obtained when a single annotator's labels, instead of the consensus, are treated as ground truth, under which precision for right-leaning labels on Bluesky falls to 0.33. The baseline model without ideology measures does not use the labels, so its platform difference on these cascades ($0.417$ for breadth and $-0.321$ for depth) is the same in every draw; what label noise can change is how much of that difference composition and alignment account for. Table~\ref{tab:noise} reports this share. With the original labels it is 84\% for breadth and 86\% for depth. In the median draw of each process it stays between 70\% and 86\%, and in the least favorable of all 400 draws it is 66\%. The pooled process lowers the share the most, and the per-platform and resampled processes, which are closest to the error we measured, leave it near its original value. Label noise therefore lowers the share under some assumptions, but composition and alignment account for most of the platform difference in every simulation. All fits use Huber's proposal 2 scale, as in Table~\ref{tab:model_comparison}.}
...
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{Breadth} & \multicolumn{2}{c}{Depth} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Error process & Median & Worst & Median & Worst \\
\midrule
Original labels & \multicolumn{2}{c}{84\%} & \multicolumn{2}{c}{86\%} \\
Per-platform rates & 86\% & 81\% & 84\% & 78\% \\
Pooled rates & 74\% & 71\% & 70\% & 66\% \\
Resampled validation & 86\% & 72\% & 84\% & 69\% \\
Single annotator & 79\% & 72\% & 76\% & 69\% \\
\bottomrule
\end{tabular}
}
\caption{\rev{Share of the baseline platform difference that composition and alignment account for under simulated label noise, 100 draws per error process. ``Worst'' is the least favorable draw.}}
\label{tab:noise}
\end{table}
...
\rev{Two differences may contribute to this gap. First, the samples and the measured behavior differ: both prior studies infer ideology from the news domains users share, across users who need not post about politics at all, whereas our labels summarize what each account writes within a conversation mentioning Biden or Trump during one campaign month. Second, part of the gap is measurement error. Applying a prevalence correction that inverts the measured confusion matrix lowers the estimated right-leaning share on Bluesky from 24.2\% to 17.9\% and the left-leaning share on Truth Social from 11.5\% to 1.9\%. Both corrections move in the expected direction, but for Bluesky a substantial gap remains after correction, so label error alone does not account for it. The remaining discrepancy may partly reflect differences in sampling frame, since a candidate-centered conversation sample is not a sample of the platform population. These corrected values are indicative rather than precise, because the correction inherits the imprecision of the minority cells in Table~\ref{tab:validation}.}

\section{\rev{Descriptive Statistics}}

\rev{Table~\ref{tab:descriptives} gives the full size distribution behind the main-text summary in Table~\ref{tab:descriptives_main}.}
...
\toprule
 & & & & \multicolumn{4}{c}{\rev{Cascades by size, $n$ (\%)}} \\
\cmidrule(lr){5-8}
Type & Platform & Cascades & Posts & 1 & 2--5 & 6--50 & $>$50 \\
\midrule
Reply & Bluesky & 79{,}348 & 199{,}413 & \rev{56{,}552 (71.3)} & \rev{16{,}823 (21.2)} & \rev{5{,}710 (7.2)} & \rev{263 (0.3)} \\
Reply & Truth Soc. & 43{,}841 & 1{,}369{,}497 & \rev{18{,}967 (43.3)} & \rev{11{,}057 (25.2)} & \rev{9{,}868 (22.5)} & \rev{3{,}949 (9.0)} \\
Repost & Bluesky & 25{,}550 & 184{,}807 & \rev{11{,}821 (46.3)} & \rev{8{,}584 (33.6)} & \rev{4{,}546 (17.8)} & \rev{599 (2.3)} \\
Repost & Truth Soc. & 218{,}579 & 3{,}847{,}903 & \rev{112{,}266 (51.4)} & \rev{64{,}991 (29.7)} & \rev{30{,}490 (13.9)} & \rev{10{,}832 (5.0)} \\
\bottomrule
\end{tabular}
}
\caption{\rev{Cascade counts and size distributions. For reply cascades, size is the number of posts in the thread including the root. For repost cascades, size is the number of reposts, excluding the root post.}}
\label{tab:descriptives}
\end{table*}

\section{\rev{Robustness of Repost Cascade Reconstruction}}

\rev{Any reconstruction rule could in principle shape the structural comparison \citep{deverna_cascade_2024}, so we re-derived every repost cascade under alternative rules and re-estimated the platform-by-size interaction ($\beta_3$ in Eq.~\ref{eq:eq1}) for each. Following \citet{bakshy_everyones_2011}, who report results under first, most recent, and split attribution, we compare several rules. Every rule we consider selects each repost's parent from the same candidate set: the original post, plus earlier reposters whom the reposting user follows. We compare the rule used in the paper, which attaches a repost to the original post whenever the user follows its author, with rules that choose among the earlier reposters the user follows and use the original post only when there are none: the earliest, the most recent, or a uniformly random one (40 draws). As a check, each is also run with the repost list order reversed.}

\rev{Figure~\ref{fig:repost_robustness} summarizes the result over 244{,}129 repost cascades (4.03 million reposts). Across all rule variants, counting each random draw separately, the breadth interaction lies between $-0.012$ and $+0.031$ and the depth interaction between $-0.083$ and $+0.028$. The same estimator applied to reply cascades gives $+0.170$ (breadth) and $-0.182$ (depth), exceeding the most extreme repost estimate under any rule by a factor of five and two respectively. The original reconstruction rule yields the largest depth difference, in absolute value, of any rule, and for breadth every rule gives a value within $0.031$ of zero. The similarity in repost scaling is therefore robust to the reconstruction rules examined here. (Because log breadth and log depth of small trees take few distinct values, the scale estimate of a Huber robust regression degenerates on repost cascades; the interactions in this analysis are estimated by OLS with HC3 standard errors.)}

\rev{The rule has limited room to matter in the first place: 55\% of Bluesky reposts and 59\% of Truth Social reposts have no eligible prior reposter and attach to the root under every rule, and a further 21\% and 12\% have exactly one candidate. A second concern is that the follower network is a snapshot without edge creation dates, so a follow created after a repost could produce a spurious parent. The follower lists were collected together with the posts and reposts, one day apart, on both platforms, so few follow edges should postdate the reposts they explain and the error applies to both platforms alike. As a check, under the most-recent-reposter rule we deleted 5\% of follow edges at random; the breadth interaction moves from $0.031$ to $0.028$ and the depth interaction from $0.004$ to $0.005$, both far below the reply values.}
...
\rev{Depth would mean something different if it largely measured back-and-forth exchange between the same two accounts rather than extended multi-party discussion. We therefore counted, for every reply, whether its author also wrote its grandparent post, that is, an $A \rightarrow B \rightarrow A$ pattern. Among all cascades, 9.3\% on Bluesky and 20.8\% on Truth Social contain at least one such exchange, reflecting mainly that Truth Social cascades are larger. Conditioning on cascades of depth two or more, the share is 68.8\% on Bluesky and 70.0\% on Truth Social, and such exchanges account for 20.0\% and 15.6\% of reply edges respectively. Dyadic exchanges are therefore common on both platforms to a similar degree. Whole threads between only two accounts differ more: among threads with a reply to a reply, 28.4\% on Bluesky and 6.5\% on Truth Social involve only two accounts, although Truth Social threads are much larger (a median of 18 participants, against 3). Refitting the models on threads with at least one reply after dropping every two-participant thread leaves most of the depth difference in the baseline model in place ($-0.184$, against $-0.222$ among all threads with a reply), and composition and alignment still account for a majority of it (63\% for depth and 59\% for breadth). Two-person conversations therefore do not explain the depth difference.}

\section{\rev{Motif Frequencies and the Null Model}}

\rev{Table~\ref{tab:motifcounts} reports the raw observed frequency of every ideology-labeled three-node motif on each platform, alongside which the z-scores should be interpreted. The two platforms differ by orders of magnitude in absolute motif volume, so a large z-score does not imply a common configuration.}
...
\rev{The z-scores compare each observed count with its distribution over 100 randomized graphs per platform, built by double-edge swaps within each cascade: two reply edges are chosen at random and the replies they lead to are exchanged, together with those replies' ideology labels, with $5m$ swap attempts in a cascade of $m$ edges. This holds fixed the set of cascades and their sizes, each post's number of replies, and the ideology attached to every participant, while randomizing who replies to whom. Motifs of this kind are not mutually independent: a chain of four nodes contains two overlapping three-node chains. This dependence does not bias the z-scores, because the randomized graphs are counted with the identical procedure, so overlap affects observed and expected counts alike. It does affect how several motifs are interpreted together: neighboring motif types share substructure, so their z-scores are correlated and we interpret them as a pattern across families rather than as independent evidence.}

\section{\rev{Topic Model Details}}

\rev{The topic model was fit over 125,623 root posts pooled across platforms. HDBSCAN initially assigns low-density documents to an outlier class; we applied BERTopic's outlier reduction step with default settings, which reassigns those documents to their nearest topic, leaving 21 documents (0.02\%) unassigned. Clustering yields 12 topics plus a residual outlier class. Manual labeling then produced the 11 coherent and interpretable categories used in the analysis: the residual class, which retains only 21 posts, is excluded, and two near-duplicate pro-Trump categories are treated as one. Every cascade in the modeling sample carries one of these 11 labels.}

\section{Modeling Strategy and Regression Diagnostics}
\label{app:strategy}
Initial Ordinary Least Squares (OLS) regression revealed strong heteroskedasticity, with residuals showing a fan-shaped pattern as a function of fitted values (Figure~\ref{fig:model_diag}). Residuals also exhibited heavy tails resembling a t-distribution. Although Cook's Distance analysis identified a small number of observations with relatively higher influence, the vast majority fell well below the threshold of $4/n$. \rev{The heavy-tailed, heteroskedastic residuals motivate robust estimation, and all models reported in the main text are robust linear models with the Huber loss. Because 61.3\% of reply cascades consist of a root post with no replies, and these sit exactly at the origin of the log--log scaling plot, the default median-absolute-deviation scale estimate collapses toward zero; standard errors computed from it are not interpretable. We therefore fit with Huber's proposal 2 scale, which does not degenerate here, and report a sensitivity analysis across samples and scale estimators below. The analyses estimated by OLS with HC3 errors are the repost reconstruction robustness check and the repost panels of Figure~\ref{fig:bd}, where lattice-valued outcomes make the Huber scale estimate degenerate.}
...
\rev{The specifications in Table~\ref{tab:model_comparison} are separate fits. The baseline regresses the outcome on \texttt{log\_size} interacted with \texttt{platform} (Model 1). Model 2 replaces the binary platform indicator with the three-category influencer variable. Models 3a to 3c add ideology: composition enters as the left-leaning post share with a spline (df\,=\,3) and the right-leaning post share linearly, alignment enters as a spline (df\,=\,3), and each term is interacted with \texttt{log\_size}. Model 4 adds topic category interacted with \texttt{log\_size}. Composition, alignment, topic category and influencer status are therefore never combined in a single specification. All models were estimated with the \texttt{statsmodels} package in Python, using the Huber loss with Huber's proposal 2 scale and sandwich standard errors.}
...
\footnotesize
\textit{Notes:}
L/R models the \emph{left-user share} with a spline (df=3) and the \emph{right-user share} linearly, \rev{interacted with $\log(\text{size})$}.
