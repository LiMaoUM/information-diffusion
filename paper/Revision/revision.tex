...
\rev{A limitation of this analysis is that the alignment ratio is computed from the same reply edges that determine cascade depth and breadth. Cross-ideological exchanges may contribute to longer sequential discussions, but sustained conversations may also create conditions under which additional cross-ideological exchanges occur. We therefore interpret ideological alignment as associated with cascade structure rather than as a causal determinant of it.}
...
\rev{These features bound what the analysis generalizes to. Our sampling frame is candidate-centered, so the ideological composition we observe reflects the campaign context as much as the platforms themselves and should not be read as the composition of either platform's full user base; and we do not observe whether the same structural patterns extend to other political topics or to non-political discussion. The more limited conclusion the analysis supports is that platforms with similar reply mechanics can exhibit systematically different conversational structures conditional on cascade size, and that these differences are associated with who participates and how participants interact across ideological lines. Establishing how broadly this holds will require comparable analyses across other platforms, countries, time periods, and topics.}

\rev{Platforms come and go; what carries forward is the distinction between diffusion patterns that recur across them and patterns that track a particular community. Reposts scaled alike here, replies did not, and the difference tracked who participated and how they engaged across ideological lines.}
