...
\rev{We ran this under four error processes: the measured platform-specific rates; a pooled matrix that applies each platform's dominant class the error rate measured largely on the other platform; a nested bootstrap that resamples the validation items and rebuilds the confusion matrix on each draw, carrying the uncertainty in Table~\ref{tab:validation} through; and the harsher rates measured against a single annotator, under which precision for right-leaning labels on Bluesky falls to 0.33. Relative to the baseline interactions without ideology controls ($0.419$ and $-0.321$), the median draw absorbs between 70\% and 86\% of the platform divergence depending on the error process, against 84\% and 86\% when the labels are left unperturbed, and the least favorable of the four hundred draws absorbs 66\%. The pooled-error scenario is the most damaging and the per-platform and bootstrap scenarios the least. No error process we simulated restores the platform gap. All fits here use Huber's proposal 2 scale, as in Table~\ref{tab:model_comparison}, and compare perturbed against unperturbed values, so the intervals are ranges across simulation draws rather than standard errors.}
