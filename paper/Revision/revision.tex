...
Existing studies have often aggregated cascades by content type (e.g., true vs. false news) and examined size, depth, breadth, or virality to identify broad drivers such as novelty, veracity, or emotional tone \cite{goel_structural_2016, vosoughi_spread_2018, berger_what_2012, zollo_emotional_2015}. \rev{This structural vocabulary remains in active use: recent work distinguishes broadcast-like spread from long peer-to-peer reshare chains at the scale of a billion Facebook posts \citep{gonzalezbailon_diffusion_2024}, and separates depth from breadth when assessing what platform interventions change \citep{slaughter_community_2025}.} However, most analyses focus on a single platform, limiting what we know about which aspects of diffusion are universal and which are platform-specific, especially in newer, ideologically distinct online environments \cite{cinelli_echo_2021}.
...
This approach provides full conversational context rather than isolated messages, allowing us to reconstruct entire reply cascades. For every post and reply, we also collected associated repost actions and each user’s follower list, allowing us to link diffusion patterns to the underlying network of connections. Data were collected between May 30 and June 30, 2024, a high-salience period that included the first U.S. presidential debate. \rev{Other collections covering Truth Social across the same campaign are available and use comparable endpoints \citep{shah_unfiltered_2024}.} \rev{Cascade counts and size distributions for both platforms and both cascade types are reported in Appendix Table~\ref{tab:descriptives}.}
...
This table is re-estimated relative to the submitted version. The submitted table used the default median-absolute-deviation scale, which degenerates on these data (see Sensitivity of the Modeling Specification), yielding standard errors that could not be interpreted. Point estimates shift modestly under the corrected scale, most visibly for Model 3c on breadth ($0.023$ to $0.071$).}
\end{minipage}
\end{table*}
...
Model 3 (\textit{Ideological Structure}) is decomposed into three specifications. Model 3a incorporates only the alignment ratio, Model 3b incorporates only user-level ideological composition, and Model 3c combines both. This decomposition shows that composition alone (Model 3b) reduces the divergence, yet substantial residual differences persist \rev{($0.289$ for breadth and $-0.196$ for depth)}. Alignment alone (Model 3a) likewise attenuates but does not eliminate platform differences: the size-platform interaction for breadth remains positive \rev{($0.236$)}, and the size-platform interaction for depth remains negative \rev{($-0.189$)}. By contrast, the combined model (3c) \rev{reduces both interaction terms by roughly five-sixths relative to the baseline, to $0.071$ for breadth and $-0.049$ for depth against baseline values of $0.419$ and $-0.321$. Only when both ideological composition and cross-ideological alignment are included together does the platform-specific scaling gap shrink this far, though for breadth a small positive interaction remains.} \rev{We describe this as statistical accounting rather than explanation: these covariates absorb the platform difference, \rev{which is what a mediating role would produce, and is also what several non-causal arrangements would produce; the design does not separate them.}}

Model fit statistics reinforce this conclusion. While Models 3a and 3b improve pseudo-$R^2$ and RMSE relative to the baseline, the gains are modest. The combined specification produces the strongest fit, with pseudo-$R^2$ increasing to \rev{$0.95$ for breadth and $0.91$ for depth}, with corresponding reductions in RMSE, \rev{indicating that ideological structure jointly accounts for a large share of the observed divergence.} Substantively, \rev{the platform difference is associated with both within-cascade conflict (alignment) and broader differences in the ideological makeup of user communities, and neither covariate absorbs it alone.} 
...
\rev{Because the stance labels contain measurement error, we tested whether the central mechanism result (Model 3c) survives label noise at the measured rates. \rev{Ignoring such error is not safe by default: misclassification by automated classifiers biases downstream regression even when overall accuracy is high \citep{teblunthuis_misclassification_2024}, and the same holds for language-model labels used as surrogates in social-science inference \citep{egami_imperfect_2023}. Those authors correct the bias analytically with a gold-standard subsample. We take the complementary route of propagating the measured error through the full pipeline, because our labels enter not as a single regressor but through composition, alignment, splines, and interactions, where an analytic correction is not available.} For each of 100 simulation draws, every user's label was replaced by a draw from the column-normalized confusion distribution $P(\text{human} \mid \text{model})$ estimated from Table~\ref{tab:validation}; each cascade's ideological composition and alignment ratio were then recomputed from the perturbed labels, and Model 3c was refit. Under platform-specific error rates, the platform-by-size interaction remains near zero in every draw: for breadth, median $0.012$ with a 95\% band of $[0.000, 0.038]$; for depth, median $-0.013$ with a band of $[-0.043, 0.000]$. Relative to the baseline interactions without ideology controls ($0.398$ and $-0.334$), at least 85\% of the platform divergence remains absorbed in the least favorable of the 100 draws, and about 96\% in the median draw. A deliberately harsher test using the pooled confusion matrix, which applies each platform's dominant class the error rate measured largely on the other platform, still leaves roughly 70\% of the divergence absorbed (breadth median $0.096$; depth median $-0.086$).

\rev{Both of those tests treat the measured error rates as known. Because those rates are themselves estimated from 200 items, and two cells are estimated imprecisely, we ran two further tests that do not assume them. The first is a nested bootstrap: on each draw we resample the validation items, rebuild the confusion matrix from that resample, and only then perturb labels and refit. This carries the uncertainty in Table~\ref{tab:validation}, including the wide interval on right-leaning labels for Bluesky, through into the substantive result. Across 100 draws the interaction stays near zero at the median (breadth $0.014$, depth $-0.018$, corresponding to 96\% and 94\% of the baseline divergence absorbed), and in the least favorable draw still leaves 72\% of the breadth gap and 70\% of the depth gap absorbed. The second test replaces the consensus-based error rates with the harsher rates measured against Annotator 1 alone, under which precision for right-leaning labels on Bluesky falls to 0.33. There the median draw absorbs 88\% of the breadth gap and 86\% of the depth gap, and the least favorable draw absorbs 77\% of each. Neither the imprecision of the validation sample nor the choice of a stricter reference restores the platform gap.}

\rev{One estimator note: all fits in this subsection use the same scale estimate as one another, and the quantities compared are perturbed against unperturbed fits rather than absolute levels, so the analysis is unaffected by the scale issue documented above for Table~\ref{model}. No standard errors are computed here; the intervals reported are ranges across simulation draws.} \rev{Under every misclassification process we simulated, adding label noise failed to restore the platform gap. We stop at that statement: the classical result that measurement error attenuates a regressor's measured explanatory power is not directly transferable here, since the ideology labels are multi-category, aggregated to the user level, and enter through composition, alignment, splines, and interactions.}}
...
\vspace{0.75ex}
\footnotesize
\rev{\textit{Estimator note:} entries in this table use the default median-absolute-deviation scale, as in the submitted version of Table~\ref{model}, and are reported without standard errors. The comparison of interest is between the two encodings, which share the estimator, so the scale issue documented for Table~\ref{model} affects both columns identically.}\\
\textit{Notes:}
