...
\rev{Entries are Model 3c (composition and alignment jointly) fitted with Huber's proposal 2 scale, matching Table~\ref{model}; $n = 120{,}693$ cascades for which reply-tree ideology shares could be recomputed at each threshold.}}
...
\rev{All eight models are fitted with the Huber loss and Huber's proposal 2 scale, matching Table~\ref{model}. The sample is the 120{,}693 cascades for which reply-tree ideology shares could be recomputed, so $\beta_3$ differs slightly in level from Table~\ref{model}. Bold marks the better of the two encodings within each column pair. Coefficients are reported without standard errors because only the comparison between encodings is at issue and both share the estimator.}
\end{table*}
