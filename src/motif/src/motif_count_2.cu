...
void compute_z_scores(const thrust::host_vector<unsigned int>& real_counts, const std::vector<std::vector<unsigned int>>& null_counts, const std::string& output_zscores_file) {
    std::vector<double> null_means(12, 0), null_stds(12, 0);
...
// CUDA Kernel for extracting and classifying 3-node subtrees with reduced edge types
__global__ void classify_subtrees(int *adj_offset, int *adj_list, int *edge_types, int num_nodes,  unsigned int *counts) {
    ...
    for (int i = start_idx; i < end_idx; i++) {
        ...
        for (int j = i + 1; j < end_idx; j++) {
            ...
            // Star Motif Mapping with 3 edge types
            int motif_type_star = -1;
            if (edge1_type == 0 && edge2_type == 0) motif_type_star = 0;  // Two Reposts
            else if ((edge1_type == 0 && edge2_type == 1) || (edge1_type == 1 && edge2_type == 0)) motif_type_star = 1;  // Repost & Align
            else if ((edge1_type == 0 && edge2_type == 2) || (edge1_type == 2 && edge2_type == 0)) motif_type_star = 2;  // Repost & Oppose
            else if (edge1_type == 1 && edge2_type == 1) motif_type_star = 3;  // Two Align
            else if ((edge1_type == 1 && edge2_type == 2) || (edge1_type == 2 && edge2_type == 1)) motif_type_star = 4;  // Align & Oppose
            else if (edge1_type == 2 && edge2_type == 2) motif_type_star = 5;  // Two Oppose
        ...
        // Chain Motif Mapping with 3 edge types
        ...
        for (int j = start_b; j < end_b; j++) {
            ...
            if (edge1_type == 0 && edge2_type == 0) motif_type_chain = 6;  // Two Reposts
            else if ((edge1_type == 1 && edge2_type == 0) || (edge1_type == 0 && edge2_type == 1)) motif_type_chain = 7;  // Align & Repost
            else if (edge1_type == 1 && edge2_type == 1) motif_type_chain = 8;  // Two Align
            else if ((edge1_type == 1 && edge2_type == 2) || (edge1_type == 2 && edge2_type == 1)) motif_type_chain = 9;  // Align & Oppose
            else if ((edge1_type == 2 && edge2_type == 0) || (edge1_type == 0 && edge2_type == 2)) motif_type_chain = 10;  // Oppose & Repost
            else if (edge1_type == 2 && edge2_type == 2) motif_type_chain = 11;  // Two Oppose
...
int main(int argc, char* argv[]) {
    ...
    thrust::device_vector<unsigned int> d_real_counts(12, 0);
    ...
    std::vector<std::vector<unsigned int>> null_motif_counts(NUM_NULL_MODELS, std::vector<unsigned int>(12, 0));
    ...
    for (int i = 0; i < NUM_NULL_MODELS; i++) {
        ...
        thrust::device_vector<unsigned int> d_counts(12, 0);
